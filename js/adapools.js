  ...
  $.each(data, function (i, item) {
    if (data[i].hasOwnProperty("epochSlots")) {
      ...
      if (data[i].epochSlots == null) {
        ...
        if (data[i].blocks != 0) {
          $history = "Epoch " + data[i].epoch + " - " + data[i].blocks + "/" + data[i].epochSlots + " blocks " +  ((data[i].epochSlots === data[i].blocks || data[i].blocks > data[i].epochSlots) ? (`<span class="perfect-star">🌟</span>`) : ("")) + "<br />" + $history;
        }
        else {
